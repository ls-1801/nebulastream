...
TEST_F(QueryEngineTestFixture, singleQueryWithManySourcesOneOfThemFails)
{
    ...
    for (size_t i = 0; i < numberOfSources; i++)
    {
        ASSERT_TRUE(source_ctrls[i]->wait_until_opened()) << "Source " << i << " should have been opened";
    }
    ...
    for (size_t buf = 0; buf < numberOfBuffersBeforeFailure + 2; buf++)
    {
        for (size_t i = 0; i < numberOfSources; i++)
        {
            ...
            source_ctrls[i]->inject_data(std::vector<uint8_t>(DEFAULT_BUFFER_SIZE, 0), NUMBER_OF_TUPLES_PER_BUFFER);
        }
    }
    ...
    for (size_t i = 0; i < numberOfSources; i++)
    {
        ASSERT_TRUE(source_ctrls[i]->wait_until_destroyed()) << "Source " << i << " should have been destroyed";
        ASSERT_TRUE(source_ctrls[i]->was_opened()) << "Source " << i << " should have been opened";
    }
...
TEST_F(QueryEngineTestFixture, failureDuringPipelineStop)
{
    ...
    ASSERT_TRUE(fail_ctrl->wait_for_destruction()) << "Failing pipeline should have been destroyed";
    EXPECT_FALSE(fail_ctrl->was_stopped()) << "Failing pipeline should not have been stopped gracefully (it threw)";

    /// Wait for successor pipeline to be destroyed
    ASSERT_TRUE(succ_ctrl->wait_for_destruction()) << "Successors of failing pipelines should have been destroyed";
    EXPECT_FALSE(succ_ctrl->was_stopped()) << "Successors of failing pipelines should not be stopped gracefully";

    /// The "good" predecessor should have been stopped gracefully
    EXPECT_TRUE(good_ctrl->was_stopped()) << "Predecessors of failing pipelines should have been stopped gracefully";
...
TEST_F(QueryEngineTestFixture, failureDuringPipelineStopMultipleSources)
{
    ...
    ASSERT_TRUE(pipe_ctrl->wait_for_stop()) << "Pipeline should be stopped after its predecessor source has been stopped";

    /// Sink should still be running (kept alive by the other branch)
    EXPECT_TRUE(sink_ctrl->keep_running()) << "Sink should not have been stopped as it is kept alive by the other predecessor";
    ...
    ASSERT_TRUE(fail_ctrl->wait_for_destruction()) << "Pipeline should be stopped forcefully";
    EXPECT_FALSE(fail_ctrl->was_stopped()) << "Pipeline should not be stopped gracefully (it threw during stop)";

    /// Wait for successor to be destroyed (cascade from failure)
    ASSERT_TRUE(succ_ctrl->wait_for_destruction()) << "Successor to failing should be stopped forcefully";
    EXPECT_FALSE(succ_ctrl->was_stopped()) << "Successor to failing should not be stopped gracefully";
...
TEST_F(QueryEngineTestFixture, failureDuringPipelineStopMultipleSourcesRaceBetweenFailAndEoS)
{
    ...
    ASSERT_TRUE(fail_ctrl->wait_for_destruction()) << "Pipeline should be stopped forcefully";
    ASSERT_TRUE(pipe_ctrl->wait_for_destruction()) << "Pipeline should be stopped after its predecessor source has been stopped";
    ASSERT_TRUE(succ_ctrl->wait_for_destruction()) << "Successor to failing should be stopped forcefully";
    ASSERT_TRUE(sink_ctrl->wait_for_destruction()) << "Successor to failing should be stopped forcefully";

    /// None of them should have been stopped gracefully (due to failure)
    EXPECT_FALSE(pipe_ctrl->was_stopped()) << "Pipeline should have failed because of the QEP failure";
    EXPECT_FALSE(fail_ctrl->was_stopped()) << "Pipeline should not be stopped gracefully";
    EXPECT_FALSE(succ_ctrl->was_stopped()) << "Successor to failing should not be stopped gracefully";
    EXPECT_FALSE(sink_ctrl->was_stopped()) << "Successor to failing should not be stopped gracefully";
...
TEST_F(QueryEngineTestFixture, failureDuringPipelineStartWithMultiplePipelines)
{
    ...
    ASSERT_TRUE(source_ctrl->wait_until_destroyed()) << "Query should have been terminated due to pipeline start failure";
...
TEST_F(QueryEngineTestFixture, failureDuringPipelineStartWithMultipleSources)
{
    ...
    ASSERT_TRUE(source1_ctrl->wait_until_destroyed()) << "Query should have been terminated due to pipeline start failure";
    ASSERT_TRUE(source2_ctrl->wait_until_destroyed()) << "Query should have been terminated due to pipeline start failure";
...
TEST_F(QueryEngineTestFixture, SingleQueryWithRepeatingSink)
{
    ...
    EXPECT_GE(sink_ctrl->invocations(), 4) << "Sink should have been invoked at least 4 times (1 initial + 3 repeats)";
...
TEST_F(QueryEngineTestFixture, SingleQueryWithRepeatingPipeline)
{
    ...
    EXPECT_GE(pipeline_ctrl->invocations(), 4) << "Pipeline should have been invoked at least 4 times (1 initial + 3 repeats)";

    /// Verify sink received the buffer after repeats completed
    EXPECT_GE(sink_ctrl->invocations(), 1) << "Sink should have received at least one buffer";
...
TEST_F(QueryEngineTestFixture, SingleQueryWithRepeatingSinkDuringQueryStop)
{
    ...
    EXPECT_GE(sink_ctrl->stop_calls(), 4) << "Sink stop should have been called at least 4 times (1 initial + 3 repeats)";
...
TEST_F(QueryEngineTestFixture, SingleQueryWithMultipleSinksDuringQueryStopOneIsRepeated)
{
    ...
    ASSERT_TRUE(sink1_ctrl->wait_for_stop()) << "Sink1 should have been stopped after repeat completion";
    ASSERT_TRUE(sink2_ctrl->wait_for_stop()) << "Sink2 should have been stopped";

    /// Verify sink1 stop was called 3 times (initial + 2 repeats)
    EXPECT_GE(sink1_ctrl->stop_calls(), 3) << "Sink1 stop should have been called at least 3 times (1 initial + 2 repeats)";

    /// Verify sink2 stop was called 1 time (no repeat)
    EXPECT_GE(sink2_ctrl->stop_calls(), 1) << "Sink2 stop should have been called at least 1 time";
...
TEST_F(QueryEngineTestFixture, ManyQueriesWithTwoSources)
{
    ...
    ASSERT_TRUE(sink_ctrls[0]->wait_for_buffers(2)) << "First sink should have received at least 2 buffers";
    ...
    for (size_t q = 0; q < numberOfQueries; q++)
    {
        ASSERT_TRUE(sink_ctrls[q]->wait_for_stop()) << "Query " << q << " sink should have been stopped gracefully";
    }
...
TEST_F(QueryEngineTestFixture, ManyQueriesWithTwoSourcesOneSourceFails)
{
    ...
    ASSERT_TRUE(source_ctrls[0]->wait_until_destroyed()) << "Query 0 source 0 should have been destroyed due to failure";
    ASSERT_TRUE(source_ctrls[1]->wait_until_destroyed()) << "Query 0 source 1 should have been destroyed due to failure";
    ...
    ASSERT_TRUE(source_ctrls[2]->wait_until_destroyed()) << "Query 1 source 0 should have been destroyed after stop_query";
    ASSERT_TRUE(source_ctrls[3]->wait_until_destroyed()) << "Query 1 source 1 should have been destroyed after stop_query";
    ...
    {
        ASSERT_TRUE(sink_ctrls[q]->wait_for_stop()) << "Query " << q << " should have been stopped gracefully";
    }
...
TEST_F(QueryEngineTestFixture, ManyQueriesWithTwoSourcesAndPipelineFailures)
{
    ...
    for (size_t q = 1; q < numberOfQueries; q++)
    {
        ASSERT_TRUE(pipeline1_ctrls[q]->wait_for_destruction()) << "Query " << q << " pipeline1 should have been destroyed due to failure";
    }
    ...
    ASSERT_TRUE(sink_ctrls[0]->wait_for_stop()) << "Query 0 should have been stopped gracefully";
