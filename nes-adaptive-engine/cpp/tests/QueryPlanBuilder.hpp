...
struct StrongId {
    ...
    StrongId& operator++() { ++value; return *this; }
    StrongId operator++(int) { StrongId tmp = *this; ++value; return tmp; }
};
...
using ControllerVariant = std::variant<
    std::shared_ptr<TestSourceController>,
    std::shared_ptr<TestPipelineController>,
    std::shared_ptr<TestSinkController>>;
...
struct QueryPlanBuildResult {
    ...
    ~QueryPlanBuildResult() {
        for (auto& s : owned_stages) { s.release(); }
        for (auto& s : owned_sources) { s.release(); }
    }
    ...
    std::shared_ptr<TestSourceController> get_source_controller(BuilderSourceId id) const {
        return source_controllers.at(id);
    }
    ...
    /// @throws std::out_of_range if ID not found
    std::shared_ptr<TestPipelineController> get_pipeline_controller(BuilderPipelineId id) const {
        return pipeline_controllers.at(id);
    }
    ...
    /// @throws std::out_of_range if ID not found
    std::shared_ptr<TestSinkController> get_sink_controller(BuilderSinkId id) const {
        return sink_controllers.at(id);
    }
};
    ...
    QueryPlanBuildResult build() {
        ...
        for (BuilderSourceId source_id : source_order_) {
            ...
            auto source = std::make_unique<TestSource>(
                "source-" + std::to_string(source_id.value),
                controller,
                provider_);
        ...
        for (BuilderPipelineId pipeline_id : pipeline_order_) {
            ...
            auto pipeline = std::make_unique<TestPipeline>(
                "pipeline-" + std::to_string(pipeline_id.value),
                controller);
        ...
        for (BuilderSinkId sink_id : sink_order_) {
            ...
            auto sink = std::make_unique<TestSink>(
                "sink-" + std::to_string(sink_id.value),
                controller,
                provider_);
        ...
        for (auto& s : result.owned_sources) s.release();
        result.owned_sources.clear();
        for (auto& s : result.owned_stages) s.release();
...
private:
    ...
    std::unordered_map<BuilderPipelineId, std::vector<std::variant<BuilderSourceId, BuilderPipelineId>>, BuilderPipelineId::Hash> pipeline_inputs_;
