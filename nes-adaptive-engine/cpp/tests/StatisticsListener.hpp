...
struct QueryStartEvent
{
    ...
    bool operator==(const QueryStartEvent& other) const { return worker_id == other.worker_id && query_id == other.query_id; }
};
...
struct QueryStopEvent
{
    ...
    bool operator==(const QueryStopEvent& other) const { return worker_id == other.worker_id && query_id == other.query_id; }
};
...
struct PipelineStartEvent
{
    ...
    {
        return worker_id == other.worker_id && query_id == other.query_id && pipeline_id == other.pipeline_id;
    }
};
...
struct TaskExecutionStartEvent
{
    ...
    {
        return worker_id == other.worker_id && query_id == other.query_id && pipeline_id == other.pipeline_id && task_id == other.task_id;
    }
};
...
struct TaskEmitEvent
{
    ...
    {
        return worker_id == other.worker_id && query_id == other.query_id && from_pipeline_id == other.from_pipeline_id
...
struct QueryRunningEvent
{
    ...
    bool operator==(const QueryRunningEvent& other) const { return worker_id == other.worker_id && query_id == other.query_id; }
};
...
struct QueryTerminatedEvent
{
    ...
    bool operator==(const QueryTerminatedEvent& other) const { return worker_id == other.worker_id && query_id == other.query_id; }
};
...
public:
    ...
    ~StatisticsListener() { stop_polling(); }
        ...
        poll_thread_ = std::thread(
            [this, stats_queue]()
            {
                while (!stop_polling_.load())
                {
                    ...
                    bool got = stats_queue->poll(50, event_type, worker_id, query_id, pipeline_id, to_pipeline_id, task_id);
                    if (!got)
                        continue;
    ...
    void record_query_start(WorkerId worker_id, QueryId query_id) { record(QueryStartEvent{worker_id, query_id}); }

    /// Record a QueryStop event.
    void record_query_stop(WorkerId worker_id, QueryId query_id) { record(QueryStopEvent{worker_id, query_id}); }

    /// Record a PipelineStart event.
    void record_pipeline_start(WorkerId worker_id, QueryId query_id, const PipelineId& pipeline_id)
    {
        record(PipelineStartEvent{worker_id, query_id, pipeline_id});
    }

    /// Record a PipelineStop event.
    void record_pipeline_stop(WorkerId worker_id, QueryId query_id, const PipelineId& pipeline_id)
    {
        record(PipelineStopEvent{worker_id, query_id, pipeline_id});
    }

    /// Record a TaskExecutionStart event.
    void record_task_execution_start(WorkerId worker_id, QueryId query_id, const PipelineId& pipeline_id, TaskId task_id)
    {
        record(TaskExecutionStartEvent{worker_id, query_id, pipeline_id, task_id});
    }

    /// Record a TaskExecutionComplete event.
    void record_task_execution_complete(WorkerId worker_id, QueryId query_id, const PipelineId& pipeline_id, TaskId task_id)
    {
        record(TaskExecutionCompleteEvent{worker_id, query_id, pipeline_id, task_id});
    }

    /// Record a TaskEmit event.
    void record_task_emit(
        WorkerId worker_id, QueryId query_id, const PipelineId& from_pipeline_id, const PipelineId& to_pipeline_id, TaskId task_id)
    {
        record(TaskEmitEvent{worker_id, query_id, from_pipeline_id, to_pipeline_id, task_id});
    }

    /// Record a QueryRunning event.
    void record_query_running(WorkerId worker_id, QueryId query_id) { record(QueryRunningEvent{worker_id, query_id}); }

    /// Record a QueryTerminated event.
    void record_query_terminated(WorkerId worker_id, QueryId query_id) { record(QueryTerminatedEvent{worker_id, query_id}); }

    /// Assertion helpers

    /// Expect exactly n QueryStart events within timeout.
    [[nodiscard]] bool expect_query_start(size_t n, std::chrono::milliseconds timeout = DEFAULT_TIMEOUT) const
    {
        return wait_for_count<QueryStartEvent>(n, timeout);
    }

    /// Expect exactly n QueryStop events within timeout.
    [[nodiscard]] bool expect_query_stop(size_t n, std::chrono::milliseconds timeout = DEFAULT_TIMEOUT) const
    {
        return wait_for_count<QueryStopEvent>(n, timeout);
    }

    /// Expect exactly n PipelineStart events within timeout.
    [[nodiscard]] bool expect_pipeline_start(size_t n, std::chrono::milliseconds timeout = DEFAULT_TIMEOUT) const
    {
        return wait_for_count<PipelineStartEvent>(n, timeout);
    }

    /// Expect exactly n PipelineStop events within timeout.
    [[nodiscard]] bool expect_pipeline_stop(size_t n, std::chrono::milliseconds timeout = DEFAULT_TIMEOUT) const
    {
        return wait_for_count<PipelineStopEvent>(n, timeout);
    }

    /// Expect exactly n TaskExecutionStart events within timeout.
    [[nodiscard]] bool expect_task_execution_start(size_t n, std::chrono::milliseconds timeout = DEFAULT_TIMEOUT) const
    {
        return wait_for_count<TaskExecutionStartEvent>(n, timeout);
    }

    /// Expect exactly n TaskExecutionComplete events within timeout.
    [[nodiscard]] bool expect_task_execution_complete(size_t n, std::chrono::milliseconds timeout = DEFAULT_TIMEOUT) const
    {
        return wait_for_count<TaskExecutionCompleteEvent>(n, timeout);
    }

    /// Expect exactly n TaskEmit events within timeout.
    [[nodiscard]] bool expect_task_emit(size_t n, std::chrono::milliseconds timeout = DEFAULT_TIMEOUT) const
    {
        return wait_for_count<TaskEmitEvent>(n, timeout);
    }

    /// Expect exactly n QueryRunning events within timeout.
    [[nodiscard]] bool expect_query_running(size_t n, std::chrono::milliseconds timeout = DEFAULT_TIMEOUT) const
    {
        return wait_for_count<QueryRunningEvent>(n, timeout);
    }

    /// Expect exactly n QueryTerminated events within timeout.
    [[nodiscard]] bool expect_query_terminated(size_t n, std::chrono::milliseconds timeout = DEFAULT_TIMEOUT) const
    {
    ...
    [[nodiscard]] bool wait_for_at_least(size_t n, std::chrono::milliseconds timeout = DEFAULT_TIMEOUT) const
    {
        std::unique_lock<std::mutex> lock(mutex_);
        return event_cv_.wait_for(lock, timeout, [this, n] { return count_type_unlocked<EventType>() >= n; });
    }

    /// Expect count in range [min_n, max_n] within timeout.
    template <typename EventType>
    [[nodiscard]] bool expect_count_in_range(size_t min_n, size_t max_n, std::chrono::milliseconds timeout = DEFAULT_TIMEOUT) const
    {
        std::unique_lock<std::mutex> lock(mutex_);
        /// Wait until we have at least min_n
        bool ok = event_cv_.wait_for(lock, timeout, [this, min_n] { return count_type_unlocked<EventType>() >= min_n; });
        if (!ok)
            return false;
        /// Give a small window for more events to arrive
        event_cv_.wait_for(lock, std::chrono::milliseconds(100), [this, max_n] { return count_type_unlocked<EventType>() > max_n; });
    ...
    [[nodiscard]] bool wait_for_query_running(adaptive_engine::QueryId query_id, std::chrono::milliseconds timeout = DEFAULT_TIMEOUT) const
    {
        std::unique_lock<std::mutex> lock(mutex_);
        return event_cv_.wait_for(
            lock,
            timeout,
            [this, query_id]
            {
    ...
    [[nodiscard]] bool
    wait_for_query_terminated(adaptive_engine::QueryId query_id, std::chrono::milliseconds timeout = DEFAULT_TIMEOUT) const
    {
        std::unique_lock<std::mutex> lock(mutex_);
        return event_cv_.wait_for(
            lock,
    ...
    [[nodiscard]] bool wait_for_events(size_t n, std::chrono::milliseconds timeout = DEFAULT_TIMEOUT) const
    {
        std::unique_lock<std::mutex> lock(mutex_);
        return event_cv_.wait_for(lock, timeout, [this, n] { return events_.size() >= n; });
    }
...
private:
    ...
    template <typename EventType>
    [[nodiscard]] bool wait_for_count(size_t n, std::chrono::milliseconds timeout) const
    {
        std::unique_lock<std::mutex> lock(mutex_);
        return event_cv_.wait_for(lock, timeout, [this, n] { return count_type_unlocked<EventType>() >= n; })
