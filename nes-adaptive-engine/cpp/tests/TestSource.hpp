...
public:
    ...
    [[nodiscard]] bool wait_until_opened(
    ...
    [[nodiscard]] bool wait_until_closed(
        std::chrono::milliseconds timeout = DEFAULT_TIMEOUT) const {
    ...
    [[nodiscard]] bool wait_until_destroyed(
        std::chrono::milliseconds timeout = DEFAULT_TIMEOUT) const {
        return wait_for_future(destroy_future_, timeout);
    }

    /// Check if the source was opened (non-blocking).
    [[nodiscard]] bool was_opened() const {
        return wait_for_future(open_future_, std::chrono::milliseconds(0));
    }

    /// Check if the source was closed (non-blocking).
    [[nodiscard]] bool was_closed() const {
        return wait_for_future(close_future_, std::chrono::milliseconds(0));
    }

    /// Check if the source was destroyed (non-blocking).
    [[nodiscard]] bool was_destroyed() const {
        return wait_for_future(destroy_future_, std::chrono::milliseconds(0));
    }
    ...
    [[nodiscard]] size_t buffers_emitted() const {
        return buffers_emitted_.load();
    }
        ...
        return std::visit(
            [this](auto&& arg) -> std::optional<BufferHandle> {
                ...
                if constexpr (std::is_same_v<T, TestSourceController::EoS>) {
                    ...
                    return provider_->wrap(
                        const_cast<void*>(static_cast<const void*>(arg.data.data())),
                        arg.data.size(),
                        metadata);
