...
/// Mock implementation of QueryLog for testing. Extends QueryLog to allow mock expectations
/// while still delegating to real QueryLog functionality.
class MockQueryLog final : public QueryLog
{
public:
    // Override methods to allow mocking while still calling base implementation
    bool logSourceTermination(LocalQueryId queryId, OriginId sourceId, QueryTerminationType type,
        std::chrono::system_clock::time_point timestamp) override
    {
        mockLogSourceTermination(queryId, sourceId, type, timestamp);
        return QueryLog::logSourceTermination(queryId, sourceId, type, timestamp);
    }

    bool logQueryFailure(LocalQueryId queryId, Exception exception,
        std::chrono::system_clock::time_point timestamp) override
    {
        mockLogQueryFailure(queryId, exception, timestamp);
        return QueryLog::logQueryFailure(queryId, exception, timestamp);
    }

    bool logQueryStatusChange(LocalQueryId queryId, QueryState status,
        std::chrono::system_clock::time_point timestamp) override
    {
        mockLogQueryStatusChange(queryId, status, timestamp);
        return QueryLog::logQueryStatusChange(queryId, status, timestamp);
    }

    // GMock methods for setting expectations
    MOCK_METHOD(void, mockLogSourceTermination, (LocalQueryId, OriginId, QueryTerminationType, std::chrono::system_clock::time_point));
    MOCK_METHOD(void, mockLogQueryFailure, (LocalQueryId, Exception, std::chrono::system_clock::time_point));
    MOCK_METHOD(void, mockLogQueryStatusChange, (LocalQueryId, QueryState, std::chrono::system_clock::time_point));
};
...
struct TestingHarness
{
    ...
    std::shared_ptr<MockQueryLog> status = std::make_shared<MockQueryLog>();
